...
const WeatherCard = ({ weather }) => {
  const { convertTemp } = useContext(SettingsContext);
  const temp = useMemo(() => {
    return {
    ...
    <div className="bg-gradient-to-r gap-3 from-cyan-500 to-blue-800 flex items-stretch p-6 md:px-16 rounded-3xl text-white">
      ...
      <div className="flex-auto flex flex-col justify-around items-center md:flex-row-reverse">
        ...
        <div className="flex flex-col items-center md:items-end">
          <p className="uppercase font-bold md:font-normal text-lg md:capitalize md:text-5xl lg:text-6xl">
            {weather.main}
          </p>
          <p className="capitalize text-sm md:mt-6 md:text-xl lg:text-2xl">
            {weather.description}
          </p>
        </div>
      </div>
